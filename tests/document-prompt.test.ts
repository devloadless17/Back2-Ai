...
describe('questions that hand the candidate documents', () => {
  it('fires on the Arabic civics wording the barème itself uses', () => {
    ...
    expect(has(systemPrompt('concept_level', concept, 'ar', q))).toBe(true);
  });

  it('fires on the dual and plural forms, which the papers use constantly', () => {
    expect(has(systemPrompt('concept_level', concept, 'ar', 'استخرج من المستندين'))).toBe(true);
    expect(has(systemPrompt('concept_level', concept, 'ar', 'حلّل الوثائق الآتية'))).toBe(true);
    expect(has(systemPrompt('concept_level', concept, 'ar', 'اعتماداً على الوثيقة'))).toBe(true);
  });

  it('fires on the translated editions of the same exercise', () => {
    expect(has(systemPrompt('concept_level', concept, 'fr', 'Présentez le document 1.'))).toBe(true);
    expect(has(systemPrompt('concept_level', concept, 'en', 'Using the documents below…'))).toBe(true);
  });
  ...
  it('fires whatever the question was classified as', () => {
    ...
    expect(has(systemPrompt('concept_level', concept, 'ar', q))).toBe(true);
    expect(has(systemPrompt('concept_level', comprehension, 'ar', q))).toBe(true);
  });
});

describe('questions that do not', () => {
  it('stays out of an ordinary concept question', () => {
    expect(has(systemPrompt('concept_level', concept, 'ar', 'ما معنى الاستعارة؟'))).toBe(false);
    expect(has(systemPrompt('concept_level', concept, 'fr', 'Quest-ce quune suite ?'))).toBe(false);
  });
  ...
  it('does not fire on "documentation" or similar longer words', () => {
    ...
    expect(has(systemPrompt('concept_level', concept, 'en', 'Read the documentation.'))).toBe(false);
  });
});

describe('it is added, never substituted', () => {
  it('keeps the hard rules that make every answer grounded', () => {
    const prompt = systemPrompt('concept_level', concept, 'ar', 'حلّل المستند الأوّل');
  ...
  it('keeps the tier instruction alongside it', () => {
    const prompt = systemPrompt('exact_match', concept, 'ar', 'حلّل المستند الأوّل');
